...
import type { translationObject } from './util';
                    ...
                    if (!chr.match(this.symbols.key)) {
                        if (!this.node.value.match(this.symbols.keyNames)) {
                            const err = new SyntaxError(
                                `Error parsing PO data: Invalid key name "${this.node.value}" at line ${this.lineNumber}. This can be caused by an unescaped quote character in a msgid or msgstr value.`,
                            );
